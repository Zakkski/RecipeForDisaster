...
namespace Disaster.API.Data
{
    public class UserListRepository : IUserListRepository
    {
        ...
        public IEnumerable<object> GetUserLists(int userId, bool selectRecipeLists)
        {
            ...
            // var lists = _context.UserLists.Where(x => x.UserId == userId && x.List.IsRecipe == selectRecipeLists).Include(x => x.List).ThenInclude(x => x.ListItems).ToList();
